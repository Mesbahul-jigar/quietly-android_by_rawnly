...
class EngineController extends StateNotifier<EngineState> {
  ...
  Future<void> _bootstrap() async {
    ...
    _metricsSub = _engine.metrics.listen((EngineMetrics m) {
      ...
      state = state.copyWith(running: false, clearSessionStart: true, error: error.toString());
    });
  ...
  Future<void> start() async {
    try {
      if (!await PermissionService().hasMic()) {
        final result = await PermissionService().request();
        if (!result.mic) throw StateError('Microphone permission is required.');
      }
      final eq = EqPreset.values.firstWhere(
        (e) => e.id == _prefs.getString('eq'), orElse: () => EqPreset.music);
      await _engine.setEqPreset(eq);
  ...
  Future<void> switchMode(AudioMode m, {bool auto = false}) async {
    ...
    if (state.running) {
      ...
      state = state.copyWith(running: false, clearSessionStart: true, error: e.toString());
    }
  }
